...
export async function GET(
  ...
  try {
    ...
    const task = await prisma.task.findFirst({
      ...
      include: { project: true, subtasks: true, course: true },
    });
...
async function handleUpdate(
  ...
  try {
    ...
    const { status, title, priority, estimatedMinutes, actualMinutes, dueDate, isAcademic, courseId } = body;
    ...
    if (task.entityId && status) {
      await prisma.entity.update({
        ...
        data: { status: status === "DONE" ? "COMPLETED" : "ACTIVE" },
      }).catch(() => {});
    }
